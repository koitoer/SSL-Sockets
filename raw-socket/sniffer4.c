...
/* Fin de definiciones de DHCP */


/* Definiciones de ARP */
#define	IPALEN	4		/* Length in bytes of an IP address */ 
#define	MAXHWALEN	5	/* Maximum length of a hardware address */

enum arp_hwtype {
        ARP_NETROM=0x0000,   /* Fake for NET/ROM (never actually sent) */
...
struct arp { 
	...
	u_int16_t protocol;		/* Protocol type */ 
	u_int16_t hwalen;			/* Hardware address length, bytes */ 
	enum arp_opcode opcode;		/* ARP opcode (request/reply) */ 
	u_int8_t shwaddr[MAXHWALEN];	/* Sender hardware address field */ 
};


/* Fin de definiciones de ARP */
...
void obtenerHIP(unsigned char *packet, int len){
	...
	if(ntohs(h_ethernet->h_proto) == ETH_P_IP){
		if(len >= (sizeof(struct ethhdr) + sizeof(struct iphdr))){
			...
			h_arp = (struct arp*)(packet + sizeof(struct ethhdr));
			if(ntohs(h_arp->hardware) == 0x0001)
				printf("\tARP ETHERNET \n");
			printf("\tSender MAC ADDRESS :" );
				impresionHex(h_arp->shwaddr-4,6);
			printf("\tTarget MAC ADDRESS :" );
				impresionHex(h_arp->shwaddr+6,6);
			printf("\tSender IP ADDRESS :");
				impresionFormatoIP(h_arp->shwaddr+2,4);
			printf("\tTarget IP ADDRESS :");
				impresionFormatoIP(h_arp->shwaddr+12,4);
	}else{
